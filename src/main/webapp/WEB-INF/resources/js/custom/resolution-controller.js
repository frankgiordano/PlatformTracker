...
//                    $scope.actions = newActions;
            ...
            modal.close.then(function(result) {
                if (result.answer === 'Yes') {
                    ResolutionService.deleteResolution(id).then(
                            function success(response) {
                                if (response) {
                                	$scope.messages =("Resolution " + id.id + " has been deleted.")
                                } else {
                                	$scope.messages =("Resolution " + id.id + " was unable to be deleted.")
                                }
                                $scope.back = true;
                                return;
                            ...
                            function error() {
                                $scope.errormessages = "Failed in delete resolution detail";
                            });
                } else {
                    return;
                }
    ...
    $scope.delete1 = function(id) {
        ResolutionService.deleteResolution(id).then(
            function success(response) {
                if (response) {
                	$scope.messages =("Resolution " + id + " has been deleted.")
                } else {
                	$scope.messages =("Resolution " + id + " was unable to be deleted.")
                }
                return;
