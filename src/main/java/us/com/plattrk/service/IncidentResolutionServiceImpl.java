...
/*		Long incidentGroupId = incidentResolution.getIncidentId();
		if(incidentGroupId != null){
			IncidentGroup incidentGroup = incidentGroupService.getGroup(incidentGroupId);
			if(incidentGroup != null)
				incidentResolution.setIncidentGroup(incidentGroup);
			else
				throw new RuntimeException("no incident existed with id "+incidentGroupId);
		}
		Long statusId = incidentResolution.getStatusId();
		if(  statusId  != null){
			 Status status =  resolutionRepository.getStatus(statusId);
			 if(status != null)
				incidentResolution.setStatus(status);
			else
				throw new RuntimeException("no incident existed with id "+incidentGroupId);
		}*/
